...
10. **Conventions** — does the change follow `.claude/rules/code-conventions.md`? Open it; do not review from memory. Most of its rules surface as *wrong code* and a defect scan finds them. Two surface only as *absent code* and slip that scan: a tuned number sitting in a `const` or a bare literal instead of a `[SerializeField]`, and a value copied out of a shared `ScriptableObject` to "match" it. So every `const`, `static readonly`, and bare numeric literal the diff adds inside a `MonoBehaviour` gets the tuned-vs-structural test from `csharp-conventions.md` — would changing it give a different-looking but still-correct build, or a broken one? Flag ⚠ the first case and name the field to serialize.
