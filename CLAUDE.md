...
- `Assets/Scripts/MagicWords/CLAUDE.md` — the DTO-to-view pipeline, what every kind of missing data turns into, why the emoji arrive as a TMP sprite asset rather than a font asset, the avatar cache, the staged reveal and the render-once invariant it rests on, and the one SOAP channel this task uses.
