...
**Status: the shell runs, none of the three tasks is built.** Created from the **Universal 2D** template — `Assets/Settings/Renderer2D.asset` is the renderer. The *Intended shape* section below now describes code that exists: the bootstrap scene, additive load/unload, the SOAP channels it talks over, and the three assemblies are all in place, and the menu reaches all three task scenes, which are stubs carrying only `Assets/Prefabs/TaskChrome.prefab`. `Assets/Scripts/SOAP/` holds a runtime subset ported from Kuiper-Prospector.
