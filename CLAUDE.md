...
**Status: the shell runs and Ace of Shadows is built. Magic Words and Phoenix Flame are stubs.** Created from the **Universal 2D** template — `Assets/Settings/Renderer2D.asset` is the renderer. The *Intended shape* section below describes code that exists: the bootstrap scene, additive load/unload, the SOAP channels it talks over, and the three assemblies are all in place, and the menu reaches all three task scenes. The two unbuilt scenes carry only `Assets/Prefabs/TaskChrome.prefab`. `Assets/Scripts/SOAP/` holds a runtime subset ported from Kuiper-Prospector.
