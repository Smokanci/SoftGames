...
- `Assets/Scripts/Bootstrap/CLAUDE.md` — the persistent scene's contract: what a task scene may not bring, the SOAP scene-swap channels, the shared Ember Dust ground and the two SOAP colour channels each scene tints it with, the pause overlay and the message banner and why both live on `TaskChrome` instead of here, why the banner panel is edge-to-edge, the ordering a task scene can rely on, and the FPS model/view seam.
...
- `Assets/Scripts/UI/CLAUDE.md` — the shared Ember button: the heat model and its test seam, the one style asset every button reads and the four fields in it that carry meaning a number cannot, why the parent picks the one idle glow, the commit feedback and the press-time lock behind it, the prefab contract and its label-or-glyph caption slot, the additive shader and its mask limitation, the two Editor-capture traps, and the app's two font assets with the fallback order that keeps Magic Words legible.
